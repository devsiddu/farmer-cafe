...
const BookingConfirmation = () => {
    ...
    const cancelOrder = async (itemId: string) => {
        try {
            const { data } = await axios.patch(`/api/bookings/cancel/${itemId}`);
            if (data.success) {
                setBookings((prev) =>
                    prev.map((booking) => ({
                        ...booking,
    ...
    const cancelTarget = bookings
        ...
        .find((item) => item._id === cancelConfirm);
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col items-end gap-2 shrink-0">
                                        <span className={`text-[11px] font-semibold px-2.5 py-1 rounded-lg ${statusStyles[item.status]}`}>
